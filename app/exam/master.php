...
class app
{
	...
	public function __construct(&$G)
	{
		...
		// $this->errorquestion = $this->G->make('errorquestion','exam');
	...
	public function ErrorQuestionList()
	{
		...
		switch($subaction)
		{
			case 'delerrorquestion':
			$page = $this->ev->get('page');
			$errorquestionid = $this->ev->get('errorquestionid');
			$this->errorquestion->delErrorQuestions($errorquestionid);
			$message = array(
				'statusCode' => 200,
				"message" => "操作成功",
				"callbackType" => "forward",
			    "forwardUrl" => "index.php?exam-master-errorquestionlist&page={$page}"
			);
			...
			$page = $this->ev->get('page');
			$page = $page > 0?$page:1;
			$errorquestion = $this->errorquestion->getErrorQuestionList($page,20);
			$this->tpl->assign('page',$page);
			$this->tpl->assign('errorquestion',$errorquestion);
			$this->tpl->display('errorquestionlist');
			break;
		}
	}
}
