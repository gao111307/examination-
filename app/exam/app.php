...
class app
{
	...
	public function submitError(){
	   $questionid=$this->ev->post('id');
            //$type=$this->ev->post('type');
            if($this->errorquestion->getErrorByQuestionAndUserId($questionid,$this->_user['sessionuserid']))//看看是否提交，提交过不再提交
            {
                echo 1;
            }
            else
            {
                    $this->errorquestion->errorQuestion($questionid,$this->_user['sessionuserid']);                                
                echo 0;
            }
	}
